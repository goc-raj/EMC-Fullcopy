...
trigger ContactTrigger on Contact (after Update, after insert, before insert, before update) {
    ...
    if(Trigger.isUpdate && Trigger.isAfter && (checkRecursive.runOnce()||Test.isrunningTest())) {
        Set<Id> updateCanadianMileageList =new Set<Id>();
        ...
        for(contact con : trigger.new){
            if((con.Map_Country__c == 'CANADA' && con.Map_Country__c != Trigger.oldMap.get(con.id).Map_Country__c)&&(con.Role__c != 'Admin' || con.Role__c != 'Manager') ){
                updateCanadianMileageList.add(con.id);
            }
        }
        if(checkRecursive.ConvertedFlag()){

            ContactTriggerHelper.updtcanadianmil(updateCanadianMileageList);
        }
        ...
        for(Contact currentContact : Trigger.New) {
            ...
            * */
            ...
            //if(currentContact.Role__c!=Trigger.oldMap.get(currentContact.ID).Role__c && currentContact.Role__c=='Driver/Manager' && Trigger.oldMap.get(currentContact.ID).Role__c == 'Manager'){
              if(currentContact.Role__c!=Trigger.oldMap.get(currentContact.ID).Role__c && (Trigger.oldMap.get(currentContact.ID).Role__c == 'Manager' || Trigger.oldMap.get(currentContact.ID).Role__c == 'Admin' && (currentContact.Role__c=='Driver/Manager' || currentContact.Role__c=='Driver/Admin' || currentContact.Role__c=='Driver'))){
